...
for ang in range(1,17,1):
    ...
    for I,J,K,L in itertools.combinations(V,4):
            ...
            ent = m_obj.mutual_information_2
            #eig = eigvals.sum()
            M_list.append([ang*10, ent ,f'{I[2]}_{J[2]}_{K[2]}_{L[2]}'])
...
fig = px.line(df, x="angulo", y="M", animation_frame='Virtuals', 
       title="Mutual Information NLMO H2O2",
      )
fig.show()
fig.update_layout(    yaxis_title=r'M matrix' )

fig.write_html("mutual_inf_2_OH1OH2_comb4_NLMO.html", include_mathjax='cdn')

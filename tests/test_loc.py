...
@pytest.mark.parametrize(
    "c_occ, v, c_vir",
    [(4.999999999999936, 69.99999999999824, 13.999999999999826)],
)
...
@pytest.mark.parametrize(
    "h1, p, h2, fcsd, corr",
    [(691.391722744511, -70.94801320520783, -1.0177345128964537, True, "RPA")],
)
...
@pytest.mark.parametrize(
    "j_pso, pso, hrpa", [(40.2215594660588, True, "HRPA")]
)
...
@pytest.mark.parametrize("j_pso, pso, rpa", [(98.95020755285924, True, "RPA")])
...
@pytest.mark.parametrize(
    "j_fcsd, fcsd, rpa", [(-102.22320818916083, True, "RPA")]
)
...
@pytest.mark.parametrize(
    "fcsd_iajb, FCSD, corr", [(-102.22320818915543, True, "RPA")]
)
def test_ssc_pathways(fcsd, FCSD, corr, hf_data):
    ...
    occ = loc_obj.occidx
    fcsd_ij = loc_obj.ssc_pathways(
        atom1="F1", atom2="H2", FCSD=FCSD, occ_atom1=occ, occ_atom2=occ
    )
    assert abs(fcsd_ij - fcsd) < 1e-5


@pytest.mark.parametrize("PSO, corr", [(True, "RPA")])
def test_ssc_pathways(PSO, corr, hf_data):
    ...
    occ = loc_obj.occidx
    vir = loc_obj.viridx
    ssc_iajb = loc_obj.ssc_pathways(
        atom1="F1", atom2="H2", PSO=PSO, occ_atom1=occ, occ_atom2=occ
    )
    assert abs(ssc - ssc_iajb) < 1e-5


@pytest.mark.parametrize(
    "fcsd_iajb, FCSD,  occ1, vir1, occ2, vir2",
    [(0.002397787579269773, True, [3, 4], [10, 12, 13], [0, 1], [9, 11, 14])],
)
def test_ssc_pathways(fcsd_iajb, FCSD, occ1, vir1, occ2, vir2, hf_data):
    ...
    fcsd_iajb_ = loc_obj.ssc_pathways(
        atom1="F1",
        atom2="H2",
        FCSD=FCSD,
        occ_atom1=occ1,

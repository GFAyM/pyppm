...
def hf_data():
    ...
    mf.kernel()
    rpa_obj = RPA(
        mol=mol, chkfile=mf.chkfile, mole_name="HF_test", calc_int=True
    )
...
#    """
#    mol, chkfile = hf_data
#    rpa_obj = RPA(mol=mol, chkfile=chkfile, mole_name='HF_test')
...
def test_Communicator(triplet, q_2_sum):
    ...
    mol, chkfile = hf_data
    rpa_obj = RPA(mol=mol, chkfile=chkfile, mole_name="HF_test")
    q = rpa_obj.Communicator(triplet=triplet, communicator=True)
...
def test_Communicator(triplet, q_2_sum, hf_data):
    ...
    mol, chkfile = hf_data
    rpa_obj = RPA(mol=mol, chkfile=chkfile, mole_name="HF_test")
    q = rpa_obj.M(triplet=triplet, communicator=True)
    assert abs(q_2_sum - (q**2).sum()) < 1e-5
...
@pytest.mark.parametrize(
    " atm_id, pert_fc_sum ", [([0], [0.17070183232781733])]
)
...
@pytest.mark.parametrize(
    " atm_id, pert_pso_squared_sum ", [([0], [0.5583537])]
)
...
@pytest.mark.parametrize(
    " atm_id, fcsd_integrals ", [(1, [1842.4910058456317])]
)
...
@pytest.mark.parametrize(
    " atm_id, pert_fcsd_squared_sum ", [([0], [0.1008467])]
)
...
@pytest.mark.parametrize(
    "atm1, atm2, FC_contribution ", [("H", "F", 208.10695206)]
)
...
@pytest.mark.parametrize(
    "atm1, atm2, PSO_contribution ", [("H", "F", 176.34185676)]
)
...
@pytest.mark.parametrize(
    "atm1, atm2, FCSD_contribution", [("H", "F", 172.7419156)]
)
...
def test_elements(atm1lst, atm2lst, fc, h1, m2, h2, hf_data):
    ...
    mol, chkfile = hf_data
    rpa_obj = RPA(mol=mol, chkfile=chkfile, mole_name="HF_test")
    h1_, m_, h2_ = rpa_obj.elements(atm1lst=atm1lst, atm2lst=atm2lst, FC=fc)
...
def test_elements(atm1lst, atm2lst, fcsd, h1, m2, h2, hf_data):
    ...
    h1_, m_, h2_ = rpa_obj.elements(
        atm1lst=atm1lst, atm2lst=atm2lst, FCSD=fcsd
    )
    assert abs((h1_**2).sum() - h1) < 1e-5
    assert abs((h2_**2).sum() - h2) < 1e-5
    assert abs((m_**2).sum() - m2) < 1e-5
...
def test_elements(atm1lst, atm2lst, pso, h1, m2, h2, hf_data):
    ...
    mol, chkfile = hf_data
    rpa_obj = RPA(mol=mol, chkfile=chkfile, mole_name="HF_test")
    h1_, m_, h2_ = rpa_obj.elements(atm1lst=atm1lst, atm2lst=atm2lst, PSO=pso)

...
def test_extraer_coeff():
    ...
    mol_t, mo_energy, mo_coeff_t, mo_occ_t, irrep_labels, spins = (
        tools.molden.load(molden)
    )
    print(mo_occ)
    print(mo_occ_t)
...
@pytest.mark.parametrize("atm_id, hibridization_coeff", [(36, 0.53788694)])
...
@pytest.mark.parametrize("atm_id", [3])
def test_mo_hibridization_2(atm_id):
    ...
    hibri = extra_func_obj.mo_hibridization_2(
        mo_label="H3", lim1=0.3, lim2=8, vir=False
    )
    assert hibri[0] == atm_id


@pytest.mark.parametrize("atm_id, hibridization_coeff", [(36, 0.45948969)])

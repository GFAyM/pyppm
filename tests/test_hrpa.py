...
def hf_data():
    ...
    mf.kernel()
    pp = HRPA(mol=mol, chkfile=mf.chkfile, mole_name="HF_test", calc_int=True)
...
def test_kappa(i, kappa2, hf_data):
    ...
    """
    mol, chkfile = hf_data
    pp = HRPA(mol=mol, chkfile=chkfile, mole_name="HF_test")
...
@pytest.mark.parametrize(
    "atmlst, pert, correction", [([0], "FC", -5.421213435194959)]
)
...
@pytest.mark.parametrize(
    "atmlst, pert, correction", [([0], "PSO", 0.32347835225943505)]
)
def test_correction_pert(atmlst, pert, correction, hf_data):
    ...
    if pert == "PSO":
        correction_ = (
            pp.correction_pert(atmlst=atmlst, PSO=True)[0] ** 2
        ).sum()
    assert abs(correction_ - correction) < 1e-5


@pytest.mark.parametrize(
    "atmlst, pert, correction", [([0], "FCSD", 10.27766223804352)]
)
def test_correction_pert(atmlst, pert, correction, hf_data):
    ...
    if pert == "FCSD":
        correction_ = (
            pp.correction_pert(atmlst=atmlst, FCSD=True)[0] ** 2
        ).sum()
    assert abs(correction_ - correction) < 1e-5


@pytest.mark.parametrize(
    "atmlst, pert, correction", [([0], "FC", -2.475781384137429)]
)
...
@pytest.mark.parametrize(
    "atmlst, pert, correction", [([0], "PSO", 1.192706252508168)]
)
def test_correction_pert_2(atmlst, pert, correction, hf_data):
...
@pytest.mark.parametrize(
    "atmlst, pert, correction", [([0], "FCSD", 3.942935194771017)]
)
def test_correction_pert_2(atmlst, pert, correction, hf_data):
    ...
    if pert == "FCSD":
        correction_ = (
            pp.correction_pert_2(atmlst=atmlst, FCSD=True)[0] ** 2
        ).sum()
    assert abs(correction_ - correction) < 1e-5


@pytest.mark.parametrize(
    "atm1lst, atm2lst, fc_response", [([0], [1], -1.2714273e-08)]
)
...
@pytest.mark.parametrize(
    "atm1lst, atm2lst, pso_response", [([0], [1], 4.5106034805326434e-09)]
)
...
@pytest.mark.parametrize(
    "atm1lst, atm2lst, fcsd_response", [([0], [1], 1.3137210541617598e-07)]
)
...
@pytest.mark.parametrize(
    "atom1, atom2, ssc, fc", [("F", "H", -182.17405198617374, True)]
)
...
@pytest.mark.parametrize(
    "atom1, atom2, ssc, fcsd", [("F", "H", -121.42222653182456, True)]
)
...
@pytest.mark.parametrize(
    "atom1, atom2, ssc, pso", [("F", "H", 43.08679427289697, True)]
)
...
def test_elements(atm1lst, atm2lst, fc, h1, m2, h2, hf_data):
    ...
    mol, chkfile = hf_data
    pp = HRPA(mol=mol, chkfile=chkfile, mole_name="HF_test")
    h1_, m_, h2_ = pp.elements(atm1lst=atm1lst, atom2lst=atm2lst, FC=fc)
...
def test_elements(atm1lst, atm2lst, fcsd, h1, m2, h2, hf_data):
    ...
    mol, chkfile = hf_data
    pp = HRPA(mol=mol, chkfile=chkfile, mole_name="HF_test")
    h1_, m_, h2_ = pp.elements(atm1lst=atm1lst, atom2lst=atm2lst, FCSD=fcsd)
...
def test_elements(atm1lst, atm2lst, pso, h1, m2, h2, hf_data):
    ...
    mol, chkfile = hf_data
    pp = HRPA(mol=mol, chkfile=chkfile, mole_name="HF_test")
    h1_, m_, h2_ = pp.elements(atm1lst=atm1lst, atom2lst=atm2lst, PSO=pso)

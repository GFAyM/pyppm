...
if os.path.exists('cloppa_fc_pzoa_vir_C2H6_ccpvdz.txt'):
    os.remove('cloppa_fc_pzoa_vir_C2H6_ccpvdz.txt')
...
for ang in range(0,19,1):
	...
	m = cloppa_obj.M(triplet=True, pzoa=True)
	p = np.linalg.inv(m)
	...
	for i,ii in vir_lmo:
		for j,jj in vir_lmo:
			...
			#if abs(ssc) > 0.001 :
			ssc_tot += ssc
			with open('cloppa_fc_pzoa_vir_C2H6_ccpvdz.txt', 'a') as f:

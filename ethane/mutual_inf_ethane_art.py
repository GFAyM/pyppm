...
for ang in range(0,19,1):
    ...
    with open(text, 'a') as f:
        f.write(f'{ang*10} {ent_ia} {ent} {ent_jb} {mutual} \n') #{np.round(cruzada, decimals=10)}

df = pd.read_csv(text, sep='\s+', header=None)

df.columns = ['ang', 'ent_iaia', 'ent_ab', 'ent_jbjb', 'mutual']

plt.figure(figsize=(10,10))
plt.plot(df.ang, df.mutual, 'b>-', label='ia') #f'a={orb1} b={orb2}')
#plt.title(r'$S_{ia}(1)$')
plt.xlabel('Dihedral angle')
plt.ylabel('Entanglement')
#ax1.legend()
plt.title(r'''Mutual Information between excitations that contribute the most''')#, 2p$_y$ y 2p$_z$')
plt.savefig('ethane_entanglement_art.png')
plt.show()  
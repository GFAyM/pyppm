...
for ang in range(0,19,1):
	...
	for i, ii in vir_lmo:
		for j, jj in vir_lmo:
			...
			m_iajb = m[H3_1s_occ[ang],i[ang]-cloppa_obj.nocc, H7_1s_occ[ang], j[ang]-cloppa_obj.nocc]
			p = p.reshape(cloppa_obj.nocc,cloppa_obj.nvir,cloppa_obj.nocc,cloppa_obj.nvir)
			p_iajb = p[H3_1s_occ[ang],i[ang]-cloppa_obj.nocc, H7_1s_occ[ang], j[ang]-cloppa_obj.nocc]

			#if abs(ssc) > 0.001 :
			with open('cloppa_M_vir_C2H6_ccpvdz.txt', 'a') as f:
				f.write(f'{ang*10}  {ii} {jj} {m_iajb} {p_iajb} \n')

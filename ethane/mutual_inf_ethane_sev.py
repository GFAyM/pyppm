...
vir_lmo = [(H3_1s, H7_1s,'1s'),(H3_2s, H7_2s, '2s'), (H3_2px, H7_2px,'2px'), (H3_2py, H7_2py,'2py'), (H3_2pz,H7_2pz,'2pz')]

for ang in range(0,19,1):
    for i,j in itertools.combinations(vir_lmo,2):
        mol_loc, mo_coeff_loc, mo_occ_loc = extra_functions(molden_file=f"C2H6_{ang*10}_ccpvdz_Cholesky_PM.molden").extraer_coeff

        vir1 = [i[0][ang], j[0][ang]]

        vir2 = [i[1][ang], j[1][ang]]
        ...
        ent_iajb = m_obj.entropy_iajb_mixedstate

        with open('mutual_information_C2H6_2exc.txt', 'a') as f:
            f.write(f'{ang*10} {I} {ent_ia} {ent_jb} {ent_iajb} {i[2]} {j[2]}   \n')

...
inv_prop = M_matrix(mol=mol_loc, mo_coeff=mo_coeff_loc, mo_occ=mo_occ_loc,
...
ent_iajb = inv_prop.entropy_iajb_single

print(inv_prop.m_iajb.sum(axis=1)) #axis=0:columna, 1:filas

def m(i,a,j,b):
	inte = -ao2mo.general(mol_loc, [mo_coeff_loc[:,[i]],mo_coeff_loc[:,[j]],mo_coeff_loc[:,b],mo_coeff_loc[:,a]], compact=False)
	#print(inte)
	#inte = inte.reshape(len(a),len(a))
	#print(inte)
	inte -= ao2mo.general(mol_loc, [mo_coeff_loc[:,[j]],mo_coeff_loc[:,a],mo_coeff_loc[:,[i]],mo_coeff_loc[:,b]], compact=False)
	return inte

print(m(H3_1s_occ[ang], [ H3_1s[ang]],H7_1s_occ[ang],[ H7_2s[ang]]))

...
text = 'cloppa_fc_vir_C2H6_ccpvdz_A1+B1.txt'
...
vir_lmo1 = ['H3_1s']#, 'H3_2s', 'H3_2pz', 'H3_2py', 'H3_2px']
vir_lmo2 = ['H7_1s']#, 'H7_2s', 'H7_2pz', 'H7_2py', 'H7_2px']
...
plt.plot(df.ang, df_F_C.fc_ab, 'b^-', label='$^{FC}J_{ia,jb}(H-H)$' )#f'a={orb1} b={orb2}')
#plt.plot(ang, DSO, 'm--', label='DSO')
plt.plot(df.ang, df.fc, 'go-', label='$^{FC}J_{ij}(H-H)$')
...
plt.suptitle('FC (A(1) + B(1)) contribution to $^3J(H-H)_{ia,jb}$ en C$_2$H$_6$, cc-pVDZ')
plt.title(f'i=C-H$_1$ a=C-H*$_1$ (1s,2s,2p$_xyz$) j=C-H$_1$. b=C-H*$_2$ (1s,2s,2p$_xyz$)')# f'a={orb1}, b={orb2}')
#plt.set_size_inches(6.5, 6.5)
               
#plt.savefig(f'FC_a1+b1_vir_C2H6.png', dpi=200)
plt.show()
...
for ang in range(1,18,1):
    mol, mo_coeff, mo_occ = extra_functions(molden_file=f"H2O2_mezcla_{ang*10}.molden").extraer_coeff
    ...
    for i,j,k,l in itertools.combinations(V,4):
        inv_prop = inverse_principal_propagator(mol=mol, mo_coeff=mo_coeff, 
                                            o1=occ_1, o2=occ_2, 
                                            v1=[i[0],j[0],k[0],l[0]], v2=[i[1],j[1],k[1],l[1]])
        m = inv_prop.M
        m_list.append([ang*10, np.sum(m), f'{i[2]}_{j[2]}_{k[2]}_{l[2]}', "Entropía"])
        m_diag_list.append([ang*10, np.sum(np.diag(m)), f'{i[2]}_{j[2]}_{k[2]}_{l[2]}', "Entropía"])


df = pd.DataFrame(m_list, columns=['angulo',  'Entanglement', 'Virtuals',  'Mutual Information'])
...
fig.write_html("m_ppe_h2o2_4exc.html", include_mathjax='cdn')
#

df = pd.DataFrame(m_diag_list, columns=['angulo',  'Entanglement', 'Virtuals',  'Mutual Information'])
fig = px.line(df, x="angulo", y="Entanglement",  animation_frame='Virtuals', color='Mutual Information',
       title="Mutual Information in the Difluorethane using combinations of 3 virtuals as antibondings, and base cc-pvdz",
      )
fig.update_layout(    yaxis_title=r'Entrelazamiento' )

fig.write_html("m_diag_ppe_h2o2_4exc.html", include_mathjax='cdn')

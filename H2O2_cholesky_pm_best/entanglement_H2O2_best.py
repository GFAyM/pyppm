...
for ang in range(1,18,1):
    ...
    orbital_1 = extra_functions(
        molden_file=f"H2O2_mezcla_{ang*10}.molden").mo_hibridization_for_list_several('H3 2', .7, 1)
    orbital_2 = extra_functions(
        molden_file=f"H2O2_mezcla_{ang*10}.molden").mo_hibridization_for_list_several('H4 2', .7, 1)
    ...
    data.append([ang*10, mutual_information,  "Entropía"])


df = pd.DataFrame(data, columns=['angulo',  'Entanglement',   'Mutual Information'])
fig = px.line(df, x="angulo", y="Entanglement",  color='Mutual Information',
...
fig.write_html("mutual_inf_h2o2_best.html", include_mathjax='cdn')
#
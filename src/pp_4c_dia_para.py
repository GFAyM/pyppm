...
#        a = numpy.diag(e_ia.ravel()).reshape(nocc,nvir,nocc,nvir)
        ...
        a += numpy.einsum('iajb->iajb', eri_mo[:nocc,nocc:,:nocc,nocc:])
        a -=  numpy.einsum('jaib->iajb', eri_mo[:nocc,nocc:,:nocc,nocc:])

        m_y = numpy.zeros((nocc*nvir,nocc*nvir), dtype='complex128')
        numpy.fill_diagonal(m_y,e_ia.ravel())
        m_y = m_y.reshape(nocc,nvir,nocc,nvir)

        
        m_y +=  numpy.einsum('iabj->iajb', eri_mo[:nocc,nocc:,nocc:,:nocc])
        m_y -=  numpy.einsum('ijba->iajb', eri_mo[:nocc,:nocc,nocc:,nocc:])
        m_y -= numpy.einsum('iajb->iajb', eri_mo[:nocc,nocc:,:nocc,nocc:])
        m_y +=  numpy.einsum('jaib->iajb', eri_mo[:nocc,nocc:,:nocc,nocc:])
        
        m = a.reshape(nocc*nvir,nocc*nvir)
        m_y = m_y.reshape(nocc*nvir,nocc*nvir)
    ...
    def pp_ssc_4c(self):
        ...
        m_inv = numpy.linalg.inv(p_xz)
        #print(m_inv.size)
        #print(numpy.isclose( m_xz, m_inv, atol=1e-1000))
        print(m_xz[34][105])
        print(m_inv[34][105])
        ...
        for c,d in nuc_pair:
            e = numpy.einsum('xai,iajb,ybj->xy', h1[atm1dic[c]].conj(), p_xz.conj(), h2[atm2dic[d]]) * 2
            para.append(e.real)
            e_y = numpy.einsum('xai,iajb,ybj->xy', h1[atm1dic[c]].conj(), p_y.conj(), h2[atm2dic[d]]) * 2

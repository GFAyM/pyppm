...
class Prop_pol:
    ...
    def M(self):
        ...
        #TESTEAR ESTO
        probando = -scipy.linalg.inv(m)
        print("juan-inversa",numpy.abs(probando.trace()),"\n")
        print("juan-matriz",numpy.abs(m.trace()),"\n")
    ...
    def pp_ssc_4c(self):
        ...
        # El valor absoluto de cada elemento se mantiene igual en cada corrida
        ...
        #TRAZA INVARIANTE - AUCAR (p_xz)
#        p_xz = -scipy.linalg.inv(m_xz)
        p_xz = -scipy.linalg.inv(m_xz) #Siempre da distintoo! (el absoluto de la traza)
#        print("juan-inversa",numpy.abs(p_xz.trace()),"\n")
#        print("juan-matriz",numpy.abs(m_xz.trace()),"\n")

        p_xz = p_xz.reshape(nocc,nvir,nocc,nvir)

        #TRAZA INVARIANTE - AUCAR (p_y)
...
#            e = numpy.einsum('xai,iajb,ybj->xy', h1[atm1dic[i]], p_xz.conj(), h2[atm2dic[j]].conj()) * 2
            e = numpy.einsum('xai,iajb,ybj->xy', h1[atm1dic[i]], p_xz, h2[atm2dic[j]].conj()) * 2
            para.append(e.real)
#            e_y = numpy.einsum('xai,iajb,ybj->xy', h1[atm1dic[i]], p_y.conj(), h2[atm2dic[j]].conj()) * 2
            e_y = numpy.einsum('xai,iajb,ybj->xy', h1[atm1dic[i]], p_y, h2[atm2dic[j]].conj()) * 2

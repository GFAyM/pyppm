...
m_obj_new = M_matrix(occ=[H3_1s_occ[ang], H4_1s_occ[ang]], vir=[H3_1s[ang], H3_2s[ang],
                    H4_1s[ang], H4_2s[ang]], mo_coeff=mo_coeff_loc, mol=mol_loc)
...
ent = m_obj_new.entropy_iajb
m = m_obj_new.m

def prueba(mol, mo_coeff, o1,o2,v1,v2):
    m = -ao2mo.general(mol, [mo_coeff[:,]])

print(np.isclose(m_obj_new.m_iajb, m_obj.m_iajb_mixedstate))

...
for ang in range(0,18,1):
	...
	ssc_ij = cloppa_obj.kernel_pathway(FC=True, FCSD=False, PSO=False,
											princ_prop=p,n_atom1=[2],  n_atom2=[3], all_pathways=True)
	for i, ii in occ_lmo:
		for j, jj in occ_lmo:
			for a, aa in lmo_vir:
				for b, bb in lmo_vir:
					ssc = cloppa_obj.kernel_pathway(FC=True, FCSD=False, PSO=False,
													...
													n_atom2=[3], occ_atom2=j[ang], vir_atom2=b[ang])
					with open(file, 'a') as f:
						f.write(f'{ang*10} {ssc_ij[0]} {ssc[0]} {ii} {aa} {jj} {bb} \n')

...
fig.write_html("mutual_inf_triplet_difluorethane_5exc.html", include_mathjax='cdn')
#
...
fig.write_html("mutual_inf_singlet_difluorethane_5exc.html", include_mathjax='cdn')
...
for ang in range(0,19,1):
    for i,j,k,l,m,n in itertools.combinations(V,6):
        ...
        mutual_information = inv_prop.mutual_information
        data.append([ang*10, mutual_information, f'{i[2]}_{j[2]}_{k[2]}_{l[2]}_{m[2]}_{n[2]}', "Entropía"])

df = pd.DataFrame(data, columns=['angulo', 'Entanglement', 'Virtuals',  'Mutual Information'])
fig = px.line(df, x="angulo", y="Entanglement", animation_frame='Virtuals', color='Mutual Information',
       title="Mutual Information Triplet in the Difluorethane (opt-HF) using combinations of 6 virtuals as antibondings, base cc-pvdz",
      )
fig.update_layout(    yaxis_title=r'Entrelazamiento' )

fig.write_html("mutual_inf_triplet_difluorethane_6exc.html", include_mathjax='cdn')
#
...
df = pd.DataFrame(data, columns=['angulo', 'Entanglement', 'Virtuals',  'Mutual Information'])
fig = px.line(df, x="angulo", y="Entanglement", animation_frame='Virtuals', color='Mutual Information',
       title="Mutual Information Singlet in the Difluorethane (opt-HF) using combinations of 6 virtuals as antibondings, base cc-pvdz",
      )
fig.update_layout(    yaxis_title=r'Entrelazamiento' )

fig.write_html("mutual_inf_singlet_difluorethane_6exc.html", include_mathjax='cdn')
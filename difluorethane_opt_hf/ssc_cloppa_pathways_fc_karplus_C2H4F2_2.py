...
for ang in range(10,11,1):
	...
	p = np.linalg.inv(m)        
	for i in range(cloppa_obj.nocc, cloppa_obj.nvir + cloppa_obj.nocc, 1):
		for j in range(cloppa_obj.nocc,cloppa_obj.nvir + cloppa_obj.nocc, 1):
			ssc = cloppa_obj.kernel_pathway(FC=True, FCSD=False, PSO=False,
											princ_prop=p,
											n_atom1=[2], occ_atom1=F3_2pz[ang], vir_atom1=i, 
											n_atom2=[6], occ_atom2=F7_2pz[ang], vir_atom2=j)
			ssc_tot += ssc
			with open('cloppa_fc_virt_C2H4F2_ccpvdz_ab.txt', 'a') as f:
				f.write(f'{ang*10} {ssc[0]} {i} {j} \n')     
			#if abs(ssc) > 1:

...
if os.path.exists('entanglement_c2h4f2.txt'):
    os.remove('entanglement_c2h4f2.txt')
...
for ang in range(0,19,1): 
    ...
    inv_prop = M_matrix(mol=mol, mo_coeff=mo_coeff, 
                occ = [  F3_2s[ang], F3_2pz[ang],  F7_2pz[ang], F7_2s[ang]],
                vir = [ v1_1[ang],v3_1[ang], v7_1[ang],v8_1[ang],
                        v1_2[ang],v3_2[ang], v7_2[ang],v8_2[ang]])
    #m = inv_prop.m_iajb
    ...
    with open('entanglement_c2h4f2.txt', 'a') as f:
        f.write(f'{ang*10} {ent_ia} {ent_jb} {ent_iajb} {mutual} \n')
    

text = 'entanglement_c2h4f2.txt'
...
plt.suptitle(r'''Triplet Quantum Entanglement between the LMOs 
    $i=F3-2pz; a=F3-2pz-3pz; j = F7-2pz; b=F7-2pz-3pz$''')
...
#plt.savefig('entanglement_triplet_c2h4f2_2.png')
plt.show()

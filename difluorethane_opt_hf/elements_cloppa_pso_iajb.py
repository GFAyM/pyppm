...
if os.path.exists('elementos_cloppa_pso_iajb.txt'):
	os.remove('elementos_cloppa_pso_iajb.txt')
		...
		for j,jj in occ_lmo:
			for a, aa in lmo_vir:
				for b,bb in lmo_vir:
					...
					p1, m, p2 = cloppa_obj.kernel_pathway_elements(FC=False, FCSD=True, PSO=False,
											princ_prop=p,
											n_atom1=[2], occ_atom1=i[ang], vir_atom1=a[ang] ,
											n_atom2=[6], occ_atom2=j[ang], vir_atom2=b[ang], elements=True)

					with open('elementos_cloppa_pso_iajb.txt', 'a') as f:
						f.write(f'{ang*10} {p1} {p2} {m} {ii} {aa} {jj} {bb}\n')

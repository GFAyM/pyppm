...
def integrals(i,a,j,b):
    m = -ao2mo.general(mol, [mo_coeff[:,a],mo_coeff[:,b],mo_coeff[:,j],mo_coeff[:,i]])
    m -= ao2mo.general(mol, [mo_coeff[:,i],mo_coeff[:,b],mo_coeff[:,j],mo_coeff[:,a]])
    return m


print(inv_prop.entropy_jb)
print(inv_prop.entropy_ia)

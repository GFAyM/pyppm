...
for ang in range(0,19,1): 
	...
	inv_prop = M_matrix(mol=mol, mo_coeff=mo_coeff, 
				occ = [  par_lib_1[ang], par_lib_2[ang]],
				vir = [ v1_1[ang ], v2_1[ang], v3_1[ang],
						v1_2[ang ], v2_2[ang], v3_2[ang] ])
	ent_ia += inv_prop.entropy_iaia
	ent_iajb += inv_prop.entropy_iajb
	...
	with open(file, 'a') as f:
		f.write(f'{ang*10} {ent_ia} {ent_iajb} {mutual} \n')


data_J = pd.read_csv(file, sep='\s+', header=None)

data_J.columns = ['ang', 'ent_ia', 'ent_iajb', 'mutual']

fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(14,10))

ax1.plot(data_J.ang, data_J.ent_ia, 'b>-')#f'a={orb1} b={orb2}')

plt.suptitle(r'''Triplet Quantum Entanglement in C$_2$H$_4$F$_2$ using the LMO that contribute the most to $^3J(F-F)^{FC+SD}$ ''')
...
ax3.plot(data_J.ang, data_J.mutual, 'b>-')#, label='$^{FC}J_{ij}(F-F)$' )#f'a={orb1} b={orb2}')
ax3.set_title('Mutual Information')# f'a={orb1}, b={orb2}')

ax2.set_xlabel('Dihedral angle')
ax2.plot(data_J.ang, data_J.ent_iajb, 'b>-')#, label='$^{FC}J_{ij}(F-F)$' )#f'a={orb1} b={orb2}')
ax2.set_title('S$_{iajb}$')# f'a={orb1}, b={orb2}')

plt.savefig('entanglement_triplet_c2h4f2_lig1_lig2_parlib1_parlib2.png')
plt.show()

...
for ang in range(1,18,1):
    ...
    for I,J,K,L,M in itertools.combinations(V, 5):
            i = [I[0],J[0],K[0],L[0],M[0]]
            j = [I[1],J[1],K[1],L[1],M[1]]
            ...
            m = m_obj.m_iajb_mixedstate_2
            #m = np.linalg.eigvals(m)
            #print(np.real(np.exp(m).sum()))
            #m = m.sum()

            M_list.append([ang*10, abs(np.diag(m).sum()) ,f'{I[2]}_{J[2]}_{K[2]}_{L[2]}_{M[2]}'])
...
fig.write_html("m_iajb_OH1OH2_triplet_comb5.html", include_mathjax='cdn')
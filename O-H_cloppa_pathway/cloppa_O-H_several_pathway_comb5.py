...
for ang in range(1,18,1):
    ...
    for i,j,k,l,n in itertools.combinations(V, 5):
        path_OH = Cloppa_specific_pathways(
            ...
            v1=[i[0], j[0],k[0],l[0],n[0]],
            v2=[i[1], j[1],k[1],l[1],n[1]])        
...
#    diag_princ_prop = np.sum(np.diag(np.linalg.inv(m)))
        ...
        M_diag_list.append([ang*10, diag_m, f'{i[2]}_{j[2]}_{k[2]}_{l[2]}_{n[2]}'])
...
fig.write_html("Princ_prop_comb5.html", include_mathjax='cdn')

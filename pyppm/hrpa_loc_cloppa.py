...
@attr.s
class HRPA_loc(Cloppa):
    """Class to perform calculations of $J^{FC}$ mechanism at HRPA level of
    of approach using localized molecular orbitals through CLOPPA model. 
    This is the p-h part of SOPPA level of approah. The HRPA class
    ...
    Returns:
        ...
        coupling using HRPA
    """
    mo_coeff_loc = attr.ib(default=None, type=numpy.array,
    ...
    mo_occ_loc = attr.ib(
        default=None, type=numpy.array,
        validator=attr.validators.instance_of(numpy.ndarray)
    )

    def __attrs_post_init__(self):
        self.occidx = numpy.where(self.mo_occ_loc > 0)[0]
        self.viridx = numpy.where(self.mo_occ_loc == 0)[0]
    ...
    @property
    def fock_energy(self):
        fock_can = self.fock_matrix_canonical
        fock_loc_occ = self.orbo.T @ fock_can @ self.orbo
        fock_loc_vir = self.orbv.T @ fock_can @ self.orbv
        #mo_energy_loc_occ = numpy.diag(fock_loc_occ)
        mo_energy_loc_occ = numpy.linalg.eigvals(fock_loc_occ)
        #mo_energy_loc_vir = numpy.diag(fock_loc_vir)
        mo_energy_loc_vir = numpy.linalg.eigvals(fock_loc_vir)
        return mo_energy_loc_occ, mo_energy_loc_vir
    ...
    def kappa(self, I):
        ...
        nocc = self.nocc
        nvir = self.nvir
        #mo_energy_occ, mo_energy_vir = self.fock_energy
        ...
        orbv = self.orbv
        fock_can = self.fock_matrix_canonical
        f_iajb = numpy.zeros((nocc,nvir,nocc,nvir))
        for i in range(nocc):
            for j in range(nocc):
                for a in range(nvir):
                    for b in range(nvir):
                        f_iajb[i,a,j,b] += orbo[:,i].T @ fock_can @ orbo[:,i]
                        #f_iajb[i,a,j,b] += orbo[:,i].T @ fock_can @ orbo[:,j]
                        #f_iajb[i,a,j,b] += orbo[:,j].T @ fock_can @ orbo[:,i]                        
                        f_iajb[i,a,j,b] += orbo[:,j].T @ fock_can @ orbo[:,j]
                        f_iajb[i,a,j,b] -= orbv[:,b].T @ fock_can @ orbv[:,b]
                        f_iajb[i,a,j,b] -= orbv[:,a].T @ fock_can @ orbv[:,a]
                        #f_iajb[i,a,j,b] -= orbv[:,a].T @ fock_can @ orbv[:,b]
                        #f_iajb[i,a,j,b] -= orbv[:,b].T @ fock_can @ orbv[:,a]


        int1 = numpy.einsum("aibj->iajb", self.eri_mo[nocc:, :nocc, nocc:, :nocc])
        int2 = numpy.einsum("ajbi->iajb", self.eri_mo[nocc:, :nocc, nocc:, :nocc])
        c = numpy.sqrt((2 * I) - 1)
        K = (int1 - (((-1) ** I) * int2)) / f_iajb
        K = K * c
        if I==2:
            for i in range(nocc):
                for j in range(nocc):
    ...
    def S2(self):
        ...
        F = numpy.zeros((nvir,nocc,nvir,nocc))
        for i in range(nocc):
            for j in range(nocc):
                for a in range(nvir):
                    for b in range(nvir):
                        F[a,i,b,j] -= orbo[:,i].T @ fock_can @ orbo[:,i]
                        #F[a,i,b,j] -= orbo[:,i].T @ fock_can @ orbo[:,j]
                        #F[a,i,b,j] -= orbo[:,j].T @ fock_can @ orbo[:,i]                        
                        F[a,i,b,j] -= orbo[:,j].T @ fock_can @ orbo[:,j]
                        F[a,i,b,j] += orbv[:,b].T @ fock_can @ orbv[:,b]
                        F[a,i,b,j] += orbv[:,a].T @ fock_can @ orbv[:,a]
                        #F[a,i,b,j] += orbv[:,a].T @ fock_can @ orbv[:,b]
                        #F[a,i,b,j] += orbv[:,b].T @ fock_can @ orbv[:,a]
        S2 = numpy.einsum("aibj->iajb", 0.5 * S2 * F)
        return S2
    ...
    def kappa_2(self):
        ...
        k_1 = self.kappa(1)
        k_2 = self.kappa(2)
        F_ia = numpy.zeros((nocc,nvir))
        fock_can = self.fock_matrix_canonical
        for i in range(nocc):
            for a in range(nvir):
                F_ia[i,a] += orbo[:,i].T @ fock_can @ orbo[:,i]
                F_ia[i,a] -= orbv[:,a].T @ fock_can @ orbv[:,a] 
        ...
        for alfa in range(nocc):
            for m in range(nvir):
                ...
                kappa[alfa, m] = kappa[alfa, m] / F_ia[alfa, m]
        return kappa
